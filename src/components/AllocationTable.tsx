...
const OrderRow = React.memo(
  ({ order, onEdit, style }: {
    ...
    const typeColor = order.type === 'EMERGENCY'
      ? 'text-red-600'
      : order.type === 'OVER_DUE'
        ? 'text-amber-600'
        : 'text-gray-600';
    ...
    return (
      <div
        className="flex items-center border-b border-gray-100 text-xs hover:bg-gray-50"
        style={style}
      >
      ...
      <div className="overflow-x-auto">
        <div style={{ minWidth: TOTAL_WIDTH }}>
          <div className="flex items-center bg-gray-50 border-b border-gray-200 text-xs font-semibold text-gray-700 sticky top-0 z-10">
            {HEADERS.map(([label, width, align], columnIndex) => (
              <div
                key={columnIndex}
